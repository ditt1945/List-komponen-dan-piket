    ...
    return (
        <div className="grid gap-4 md:grid-cols-3 mb-8">
            ...
            <Card className="p-6">
                <div className="flex items-start justify-between">
                    <div className="flex-1">
                        <div className="text-sm font-medium text-white/50 mb-2">Total Komponen</div>
                        <div className="text-3xl font-bold text-white mb-1">{totalItems}</div>
                        <div className="flex items-center gap-2 text-xs">
                            <span className={`px-2 py-1 rounded-full ${checkPercentage >= 80 ? 'bg-emerald-500/20 text-emerald-400' : checkPercentage >= 50 ? 'bg-amber-500/20 text-amber-400' : 'bg-rose-500/20 text-rose-400'}`}>
                                {checkedToday}/{totalItems} di-cek
                            </span>
                            <span className="text-white/30">{checkPercentage}%</span>
                        </div>
                    </div>
            ...
            <Card className="p-6">
                <div className="flex items-start justify-between">
                    <div className="flex-1">
                        <div className="text-sm font-medium text-white/50 mb-2">Kehadiran Piket</div>
                        <div className="text-3xl font-bold text-white mb-1">
                            {totalScheduled > 0 ? `${presentToday}/${totalScheduled}` : "0"}
                        </div>
                        <div className="flex items-center gap-2 text-xs">
                            <span className={`px-2 py-1 rounded-full ${attendancePercentage === 100 ? 'bg-emerald-500/20 text-emerald-400' : attendancePercentage >= 50 ? 'bg-amber-500/20 text-amber-400' : 'bg-rose-500/20 text-rose-400'}`}>
            ...
            <Card className="p-6">
                <div className="flex items-start justify-between">
                    <div className="flex-1">
                        <div className="text-sm font-medium text-white/50 mb-2">Status Alert</div>
                        <div className="text-3xl font-bold text-white mb-1">{needsAttention}</div>
                        <div className="flex items-center gap-2 text-xs">
                            <span className={`px-2 py-1 rounded-full ${needsAttention === 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                                {needsAttention === 0 ? 'Semua Normal' : 'Perlu Perhatian'}
                            </span>
                        </div>
                    </div>
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${needsAttention === 0 ? 'bg-emerald-500/10' : 'bg-rose-500/10'}`}>
